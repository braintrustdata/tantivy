...
impl Footer {
    ...
    pub async fn extract_footer_async(file: FileSlice) -> io::Result<(Footer, FileSlice)> {
        ...
        let footer_metadata_len = <(u32, u32)>::SIZE_IN_BYTES;
        let footer_meta_slice = file.slice_from_end(footer_metadata_len);
        let footer_meta_bytes = footer_meta_slice.read_bytes_async().await?;
        let (footer_len, footer_magic_byte): (u32, u32) =
            footer_meta_bytes.as_ref().deserialize()?;
