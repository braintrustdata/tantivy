...
impl LazyFstIndex {
    ...
    pub(crate) fn lower_bound(&self, key: &[u8]) -> io::Result<Option<u64>> {
        let span = tracing::info_span!(
            "tantivy_sstable_lazy_fst_lower_bound",
            fst_bytes = self.fst_file.len() as u64,
            key_len = key.len() as u64,
            pages_loaded = tracing::field::Empty,
            bytes_loaded = tracing::field::Empty,
        );
        let _guard = span.enter();

        let result = self.lower_bound_impl(key);
        if let Ok(cache) = self.pages.lock() {
            span.record("pages_loaded", cache.pages.len() as u64);
            span.record(
                "bytes_loaded",
                cache
                    .pages
    ...
    fn lower_bound_impl(&self, key: &[u8]) -> io::Result<Option<u64>> {
